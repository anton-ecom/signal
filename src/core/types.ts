...
export interface SignalInstance<T> {
  ...
  // Methods
  reflect(message: string, caller?:any, context?: Record<string, unknown>, component?: string): SignalInstance<T>;
  layer<U = T>(name: string, context?: Record<string, unknown>): SignalInstance<U>;
  success<U = T>(value: U): SignalInstance<U>;
...
export interface SignalPlugin<TOptions = unknown> {
   ...
   */
  execute: <T>(signal: SignalInstance<T>, options?: TOptions) => SignalInstance<T>;
...
export type SignalPluginHandler = {
  ...
  use: <T>(signal: Signal<T>, pluginConfig: SignalPluginConfig | string) => Signal<T>;
};
